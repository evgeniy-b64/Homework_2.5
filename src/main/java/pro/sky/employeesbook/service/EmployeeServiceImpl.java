...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public Employee addEmployee(String firstName, String lastName, int department, int salary) {
        ...
        return checkAndAdd(employeeToAdd);
    }

    public Employee addEmployee(Employee employee){
        return checkAndAdd(employee);
    }

    private Employee checkAndAdd(Employee employee){
        if (employees.size() >= maxEmployeesNumber) {
            throw new EmployeeStorageIsFullException();     // если штат заполнен
        }
        if (checkData(employee)) { // проверка правильности ввода имени нового сотрудника
        ...
        if (employees.containsValue(employee)) {
            throw new EmployeeAlreadyAddedException("Сотрудник " + employee.getFirstName() + " " + employee.getLastName() + " уже добавлен");
        }
        String key = makeKey(employee.getFirstName(), employee.getLastName());
    ...
    public Map<String, Employee> autofill() {
        addEmployee("Александр", "Пушкин", 1, 56000);
